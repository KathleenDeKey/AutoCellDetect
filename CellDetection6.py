...
cv2.createTrackbar("Accumulator Resolution", "Enter Values", 0, 100, on_trackbar)
...
while True:
    # load images
    bfimg = cv2.imread('intensityTest.tif', cv2.IMREAD_GRAYSCALE)
    # bfimgGray = cv2.equalizeHist(bfimg)
    trimg = cv2.imread('intensityTest.tif')
    ...
    dp = cv2.getTrackbarPos("Accumulator Resolution", "Enter Values") / 100.0
    edge_threshold = cv2.getTrackbarPos("Edge Threshold", "Enter Values") / 10.0
    accumulator_threshold = cv2.getTrackbarPos("Accumulator Threshold", "Enter Values")
