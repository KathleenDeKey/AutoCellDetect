...
def find_cell_intensity(ellipse, trimg):
...
def find_corrected_cell_intensity(ellipse, cell_intensity, area, trimg):
    ((centx, centy), (minor_axis_b, major_axis_a), angle) = ellipse
    cell_mask = np.zeros_like(trimg)
    cell_mask = cv2.ellipse(cell_mask, ellipse, (255, 255, 255), thickness=cv2.FILLED)
...
def write_excel_file(cells: dict, dataset_number, bf_only):
    if len(cells) == 0:
        ...
        if bf_only:
            column_order = ['center', 'averaged diameter in pixels', 'averaged diameter', 'area', 'aspect ratio', 'taylor', 'eccentricity',
